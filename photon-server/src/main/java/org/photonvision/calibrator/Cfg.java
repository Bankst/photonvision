...
public class Cfg {
    ...
    static final boolean legacyPattern =
            true; // true is old style with aruco first, default in OpenCV is false - new style with black
    // first
    static final boolean writeBoard = true;

    // user config for convergence criteria
    static final int pt_min_markers = 1;
    static final double matchStillCidsMin =
            0.9; // exclusive, Jaccard similarity of current and previous corner ids lists for mean flow
    // calc
    static final double mean_flow_max = 2.; // exclusive
    static final double pose_close_to_tgt_min =
            0.8; // exclusive. was 0.85 overlap - the Jaccard score between shadow and actual img
    ...
    static final int wait =
            1; // (20 almost works) milliseconds to wait for user keyboard response to a new image
    static final long keyLockoutDelayMillis =
            10000L; // allows banging on the keyboard trying to get a response within "wait" and ignore
    // multiple presses
