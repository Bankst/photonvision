        ...
        for (int index = 0; index < img1ChannelBuff.length; index++) // process each element of the row
        {
            ...
            if (img1ChannelBuff[index] == 0) // is it black?
            {
                img1ChannelBuff[index] =
                        1; // make it gray - I'd like to make this 0 so it's easy to see the camera image
                // behind it but that messes up the shadow board logic that relies on non-zero pixels. Need
    ...
    void create_maps(Mat K, Mat cdist, Size sz) {
        ...
        this.Knew =
                Calib3d.getOptimalNewCameraMatrix(
                        K, cdist, sz, 1.); // .2% higher than Python for same input
        ...
        Main.Kcsv(Id.__LINE__(), this.Knew);
        // Main.LOGGER.log(Level.WARNING, "Knew " + this.Knew + "\n" + this.Knew.dump()); // same as
        // input to make_distort_map
