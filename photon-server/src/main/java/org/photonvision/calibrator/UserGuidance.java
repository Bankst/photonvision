...
public class UserGuidance {
    ...
    private void calibrate() throws Exception {
        ...
        double[] pvar_prev =
                this.calib.varIntrinsics().clone(); // calib still has the previous intrinsics
        ...
        double[] pvar =
                this.calib
                        .varIntrinsics(); // save the new intrinsics - just a shorter name to match original
    ...
    private void set_next_pose() throws Exception {
        ...
        // Main.LOGGER.log(Level.WARNING, "board_warped created r/t " + this.tgt_r.dump() +
        // this.tgt_t.dump()  + board_warped);
    }
    ...
    private double pose_close_to_tgt() {
        ...
        // Main.LOGGER.log(Level.WARNING, "pose_valid " + this.tracker.pose_valid() + ", tgt_r empty " +
        // this.tgt_r.empty());
    ...
    boolean update(boolean force) throws Exception {
        ...
        if ((this.calib.keyframes.isEmpty() && this.tracker.N_pts() >= this.allpts / 2)) {
            ...
            if (
            /*! np.isnan(this.calib.K()).any() &&*/ this.calib.reperr()
                    < this.min_reperr_init) // assume K is all numeric - no way it couldn't be
            {
    ...
    void draw(Mat img, boolean mirror)
        ...
        if (!this.tgt_r.empty()) {
            ...
            if (imgBuff.length != board_warpedBuff.length)
                throw new Exception("major trouble here"); // debug statement
    ...
    void write() {
        ...
        String calibrationDataFile =
                "CameraCalibrationData_" + simpleDateFormat.format(new Date()) + ".txt";
