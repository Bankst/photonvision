...
public class ChArucoDetector {
    ...
    public ChArucoDetector() {
        ...
        /// create board
        // this.board.setLegacyPattern(Cfg.legacyPattern); //FIXME shouldn't use - remove when we match
        // the original and we can go our own way
        ...
        if (Cfg.writeBoard) {
            ...
            Imgcodecs.imwrite(file, this.boardImage, writeBoardParams);
        ...
        Main.LOGGER.log(
                Level.CONFIG, "" + detector.getCharucoParameters().get_minMarkers()); // 2 default
        Main.LOGGER.log(
                Level.CONFIG, "" + detector.getCharucoParameters().get_tryRefineMarkers()); // false default
        Main.LOGGER.log(
                Level.CONFIG,
                "" + detector.getDetectorParameters().get_cornerRefinementMaxIterations()); // 30 default
        Main.LOGGER.log(
                Level.CONFIG,
                "" + detector.getDetectorParameters().get_cornerRefinementMethod()); // 0 default
        Main.LOGGER.log(
                Level.CONFIG, "" + detector.getRefineParameters().get_checkAllOrders()); // true default
        Main.LOGGER.log(
                Level.CONFIG, "" + detector.getRefineParameters().get_errorCorrectionRate()); // 3.0 default
    }
    ...
    public void detect_pts(Mat img) throws Exception {
        ...
        try {
            ...
            Main.LOGGER.log(Level.SEVERE, img + " " + this.ccorners + " " + this.cids + "\n" + e);
        }
        ...
        if (!this.cids.empty()
                && this.cids.rows()
                        > 0) // double check since there was some unknown failure to get the N_pts set right
        {
        ...
        { // allow comparison with last good detection - ignoring frames with no detected corners say
            // because of blur.
        ...
        if (this.p3d.empty() || this.p2d.empty())
            throw new Exception("p3d or p2d empty"); // shouldn't happen
        ...
        //     double norm = Math.sqrt(Math.pow(point[0], 2) + Math.pow(point[1], 2)); // L2 norm
        // (Frobenious)
    ...
    public void computeMeanFlow() {
        ...
        if (last_cids.rows() >= 1 && cids.rows() >= 1) // handles the first time and other cases
        {
            ...
            while (indexCurrent < this.cids.rows() && indexLast < this.last_cids.rows()) {
                if (cidsArray[indexCurrent]
                        == last_cidsArray[
                                indexLast]) { // great we have matching cids to compare then bump both to stay in
                    // sync
                    double diffX =
                            ccornersArray[2 * indexCurrent] - last_ccornersArray[2 * indexLast]; // current - last
                    double diffY =
                            ccornersArray[2 * indexCurrent + 1]
                                    - last_ccornersArray[2 * indexLast + 1]; // current - last
                    ...
                    indexLast++;
                    continue;
                } else if (cidsArray[indexCurrent]
                        < last_cidsArray[
                                indexLast]) { // missing corresponding last so bump current to catch up to last
                    indexCurrent++;
                    continue;
                } else if (cidsArray[indexCurrent]
                        > last_cidsArray[
                                indexLast]) { // missing corresponding current so bump last to catch up to current
                    indexLast++;
                    continue;
                }
            ...
            if (countMatchingCids
                            > 0 // allow some missing cids from either list by a Jaccard index similarity
                    && (countMatchingCids / (this.last_cids.rows() + this.cids.rows() - countMatchingCids))
                            > Cfg.matchStillCidsMin) {
                this.mean_flow /= countMatchingCids; // mean of the sum of the norms
            } else {
                this.mean_flow =
                        Double.MAX_VALUE; // avoided the divide by zero and return flow is big (no need to be
                // infinite)
            }
        }
    }
    ...
    public Mat get_pts3d() { return this.p3d; }
    ...
    public void update_pose() throws Exception {
        ...
        this.pose_valid =
                Calib3d.solvePnPRansac(p3dReTyped, p2dReTyped, this.K, distReTyped, rvec, tvec, false);
...
// The correct way to check calibration accuracy is to use the reprojection error provided by
// OpenCV. I'm not sure why this
// wasn't mentioned anywhere in the answer or comments, you don't need to calculate this by hand -
// it's the return value of
// calibrateCamera. In Python it's the first return value (followed by the camera matrix, etc).
// The reprojection error is the RMS error between where the points would be projected using the
// intrinsic coefficients and
// where they are in the real image. Typically you should expect an RMS error of less than 0.5px - I
// can routinely get around
