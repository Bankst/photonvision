...
public class PoseGeneratorDist {
    ...
    class Gen_Bin {
        private List<Double> lst =
                new ArrayList<>(
                        40); // always manipulate 2 items at a time instead of making a new datatype for a pair
     ...
     * @return rvec, tvec
     */
    private static List<Mat> orbital_pose(
            Mat bbox, double rx, double ry, double Z, double rz) // force caller to use rz=0 if defaulting
            {
        // Main.LOGGER.log(Level.WARNING, "method entered  . . . . . . . . . . . . . . . . . . . . . . .
        // .");
        ...
        // in Python (Ry).dot(Rx).dot(Rz) messed up nomenclature - it's often really matrix multiply Ry
        // times Rx times Rz
        ...
        Core.gemm(
                R3x3, Rz, 1., new Mat(), 0., R3x3); // rotation matrix of the input Euler Angles [radians]
        /********************************************************************************************************* */
        ...
        Mat translateToBoardCenter =
                new Mat(
                        bbox.rows(), bbox.cols(), bbox.type()); // matching bbox for element by element multiply
    ...
    private static List<Mat> pose_planar_fullscreen(Mat K, Mat cdist, Size img_size, Mat bbox) {
        ...
        // don't use the principal point throughout just have X and Y no Z until it's calculated in the
        // middle
        // compute a new Z
        // Main.LOGGER.log(Level.WARNING, "camera matrix K " + K + "\n" + K.dump());
        // Main.LOGGER.log(Level.WARNING, "cdist " + cdist.dump());
        ...
        MatOfPoint2f p =
                new MatOfPoint2f(
                        new Point(img_size.width / 2. - pB[0] / 2., img_size.height / 2. + pB[1] / 2.));
     ...
     * @return
     */
    private pose_from_boundsReturn pose_from_bounds(
            Mat src_extParm, Rect tgt_rect, Mat K, Mat cdist, Size img_sz) {
        // Main.LOGGER.log(Level.WARNING, "method entered  . . . . . . . . . . . . . . . . . . . . . . .
        // .");
        // Main.LOGGER.log(Level.WARNING, "src_extParm " + src_extParm + "\n" + src_extParm.dump()); //
        // full ChArUcoBoard size + Z
        ...
        int MIN_WIDTH =
                (int)
                        Math.floor(
                                img_sz.width
                                        / 3.333); // posed guidance board must be about a third or more of the camera,
        // screen size
        ...
        if (rot90) {
            ...
            // src_ext = src_ext.clone(); // cloning not needed in this implementation since already
            // hiding parameter src_extParm
            double swapWH = src_ext[0];
            src_ext[0] = src_ext[1];
            src_ext[1] = swapWH;

            if (tgt_rect.height < MIN_WIDTH) {
                // double scale = MIN_WIDTH / tgt_rect.width; //FIXME is this wrong in original?
                // tgt_rect.width => tgt_rect.height? - rkt
        ...
        double aspect =
                src_ext[0] / src_ext[1]; // w/h of the full ChArUcoBoard // [2520; 1680; 2520] => 1.5
        ...
        int[] min_off = {0, 0};
        int[] max_off = {
            (int) (img_sz.width - tgt_rect.width), (int) (img_sz.height - tgt_rect.height)
        };
    ...
    private Gen_Bin[] gb = {
        new Gen_Bin(Math.toRadians(-70.), Math.toRadians(70.)), // valid poses: r_x -> -70° .. 70°
        new Gen_Bin(Math.toRadians(-70.), Math.toRadians(70.))
    }; // valid poses: r_y -> -70° .. 70°
     ...
     */
    private Function<Integer, double[]> orbital =
            (axis) -> {
    ...
    private static final int SUBSAMPLE =
            20; // there is not a good conversion in general for this type of Python variable. Assuming
    // value doesn't change nor instantiated more than once and changed elsewhere this
    // conversion works.
    ...
    static {
        Main.LOGGER.log(Level.CONFIG, "Starting ----------------------------------------");
    }
    ...
    PoseGeneratorDist(Size img_size) {
        ...
        mask =
                Mat.zeros(
                        new Size(
                                Math.floor(img_size.width / this.SUBSAMPLE),
                                Math.floor(img_size.height / this.SUBSAMPLE)),
                        CvType
                                .CV_8UC1); // t() transpose not needed in Java since those bindings account for w-h
        // reversal
    }
    ...
    List<Mat> get_pose(Mat bbox, int nk, int tgt_param, Mat K, Mat cdist) throws Exception {
        ...
        Main.Kcsv(Id.__LINE__(), K);
        if (nk == 0) {
            // x is camera pointing ahead up/down; y is camera pointing left/right; z is camera rotated (Z
            // is axis from camera to target)
            // init sequence: first keyframe  0° flat - not up or down, 45° pointing left, 22.5° rotated
            // CW
        ...
        if (tgt_param < 4) {
            ...
            double[] angleIteration =
                    this.orbital.apply(
                            axis); // get the next iteration for x & y pair of angles for the given axis
            // Main.LOGGER.log(Level.WARNING, "angleIteration " + Arrays.toString(angleIteration));

            /********************************************************************************************************* */
            List<Mat> rt =
                    orbital_pose(bbox, angleIteration[0], angleIteration[1], this.orbitalZ, this.rz);
            /********************************************************************************************************* */
            ...
            if (tgt_param > 1) {
                ...
                offPPoint[tgt_param - 2] +=
                        ((tgt_param - 2) == 0 ? this.img_size.width : this.img_size.height) * 0.05 * this.sgn;
        ...
        /********************************************************************************************************* */
        Rect bounds =
                Distortion.loc_from_dist(
                        pts, dpts, this.mask, false, 1.); // ignore previously used masked off areas
        /********************************************************************************************************* */

        if (bounds == null) {
            Main.LOGGER.log(Level.SEVERE, "bounds is null, pose not contributing; guessing what to do");
            return get_pose(
                    bbox, nk, 3, K, cdist); // best guess of what the author meant to do (drop the axis)
        }
        ...
        Rect tgt_rect =
                new Rect(
                        bounds.x * this.SUBSAMPLE,
                        bounds.y * this.SUBSAMPLE,
                        bounds.width * this.SUBSAMPLE,
                        bounds.height * this.SUBSAMPLE);
        /********************************************************************************************************* */
        ...
        Mat.ones(nbounds.height, nbounds.width, this.mask.type())
                .copyTo(
                        this.mask.submat(
                                nbounds.y, nbounds.y + nbounds.height, nbounds.x, nbounds.x + nbounds.width));
